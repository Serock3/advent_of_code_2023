...
fn solve(input: &str) -> usize {
    ...
    lines
        .into_par_iter()
        .map(|line| {
            let res = solve_times_five(line);
            let mut counter = counter.lock().unwrap();
            *counter += 1;
            println!("{}/{} Done with {line}", *counter + 1, num_lines);
...
fn solve_times_five(input: &str) -> usize {
    let (condition, sizes) = input.split_once(' ').unwrap();
    let sizes = sizes
        .split(',')
        .map(|n| n.parse::<usize>().unwrap())
    ...
    let condition = std::iter::repeat(condition).take(5).join("?");
    let mut cache = HashMap::new();
    solve_recursive(&sizes, &condition.chars().collect_vec(), &mut cache)
}
...
mod tests {
    ...
    pub(crate) fn test_example_single() {
        let input = "???.### 1,1,3";
        assert_eq!(solve_times_five(input), 1)
    }

    #[test]
    pub(crate) fn test_example_single2() {
        let input = ".??..??...?##. 1,1,3";
        assert_eq!(solve_times_five(input), 16384)
    }
    ...
    pub(crate) fn test_example_single4() {
        let input = "????.#...#... 4,1,1";
        assert_eq!(solve_times_five(input), 16)
    }

    #[test]
    pub(crate) fn test_example_single5() {
        let input = "????.######..#####. 1,6,5";
        assert_eq!(solve_times_five(input), 2500)
    }
    #[test]
    pub(crate) fn test_example_single6() {
        let input = "?###???????? 3,2,1";
        assert_eq!(solve_times_five(input), 506250)
    }
